...
给定一个整数，编写一个函数来判断它是否是 2 的幂次方。[2的幂](https://leetcode-cn.com/problems/power-of-two/)
...
两个整数的 汉明距离 指的是这两个数字的二进制数对应位不同的数量。计算一个数组中，任意两个数之间汉明距离的总和。[汉明距离总和](https://leetcode-cn.com/problems/total-hamming-distance/)
...
给定范围 [m, n]，其中 0 <= m <= n <= 2147483647，返回此范围内所有数字的按位与（包含 m, n 两端点）。[数字范围按位与](https://leetcode-cn.com/problems/bitwise-and-of-numbers-range/)
