...
N个盒子（1~N），盒子i中有$A_i$个球，现需使每个盒中球的个数各不相同，盒子里面只能放入不可取出，问题：最少放入多少球，使每个盒子球数各不相同。[LeeCode945](<https://leetcode-cn.com/problems/minimum-increment-to-make-array-unique/>)
...
F(0) = A,F(1) = B ,F(i) = F(i-1) +F(i-2) i>=2 ,判断参数组中,第N项能否被3整除

输入：第一行T,表示T组数据，第二行 A B N 表示第一项和第二项的值以及要判断的第N项。

输出：如果能被3整除则输出YES否则输出NO。
...
最大公约数是gcd，gcd性质：1）gcd(a,a)=a, 2）gcd(a,b)=gcd(b,a mod b), 3） gcd(a,b,c)=gcd(gcd(a,b),c)，N个数字的数组，第i个数字为$A_i$，求：在数组所有连续区间中，数字gcd与个数的乘积的最大值

输入：第一行数组元素个数n，第二行n个元素
